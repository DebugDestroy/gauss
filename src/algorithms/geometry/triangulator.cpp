...
namespace algorithms::geometry {
    ...
    void Triangulator::logTriangle(const Triangle& tri, const std::string& prefix) const {
    ...
    void Triangulator::logEdge(const Edge& edge, const std::string& prefix) const {
        std::ostringstream oss;
    ...
    Triangulator::Triangulator(core::Logger& lg) : logger(lg) {
        logger.trace("[Triangulator] Инициализация калькулятора компонент");
    }

    std::vector<Triangle> Triangulator::bowyerWatson(const std::vector<PointD>& points, const int width, const int height) {
                ...
                for (const auto& edge : { Edge{tri.a, tri.b}, Edge{tri.b, tri.c}, Edge{tri.c, tri.a} }) {
                    bool isShared = std::any_of(badTriangles.begin(), badTriangles.end(),
                        [&](const Triangle& other) { 
                            return !(tri == other) && otherHasEdge(other, edge); 
                        });
