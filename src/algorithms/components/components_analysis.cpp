...
namespace algorithms::components {
   ...
   // Основной конструктор (для готовой матрицы)
    Component::Component(core::Logger& lg, const std::vector<std::vector<double>>& inputComponenta, int count) : logger(lg), componenta(inputComponenta), pixelCount(count) {
    ...
    for (int y = 0; y < static_cast<int>(componenta.size()); ++y) {
        for (int x = 0; x < static_cast<int>(componenta[0].size()); ++x) {
            if (std::fabs(componenta[y][x] - core::WHITE) < core::EPSILON) {
    ...
    for (int y = min_y; y <= max_y; ++y) {
        for (int x = min_x; x <= max_x; ++x) {
            if (std::fabs(componenta[y][x] - core::WHITE) < core::EPSILON) {  // Исправлено условие
